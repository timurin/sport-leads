...
## Защищённый дизайн Platform Topbar

Компонент `frontend/components/navigation/top-navigation.tsx` является утверждённым эталоном глобальной верхней панели платформы.

Компонент `frontend/components/layout/app-topbar.tsx` является оболочкой и не должен содержать альтернативную реализацию верхней панели.

Код эталона:

`DS-SHELL-02 — Platform Topbar`

`DS-SHELL-01 — Platform Sidebar` и `DS-SHELL-02 — Platform Topbar` вместе образуют единый защищённый Platform Shell.

Утверждённый состав и поведение:

* единая высота topbar — `64px` на малых экранах и `72px` начиная с `md`;
* светлый фон, нижняя граница и визуальная система SPORT-LEAD;
* название активного верхнеуровневого раздела;
* навигация внутри активного раздела;
* активный маршрут определяется через `frontend/lib/navigation.ts`;
* desktop-навигация отображается начиная с `lg`;
* на tablet и mobile используется компактное меню раздела;
* глобальная кнопка поиска;
* глобальная кнопка «Создать»;
* dropdown закрывается при повторном нажатии, выборе пункта, клике вне меню, смене маршрута и нажатии `Escape`;
* popup и dropdown не должны выходить за границы viewport;
* topbar не должен создавать горизонтальный скролл;
* title и элементы управления не должны перекрываться;
* title допускается сокращать многоточием при недостатке места;
* focus, hover и active-состояния являются частью утверждённого дизайна.

Утверждённое поведение глобального поиска:

* при нажатии поиск занимает всю доступную ширину topbar;
* поле получает фокус автоматически;
* поиск закрывается по кнопке закрытия, клавише `Escape` или после перехода по результату;
* без введённого текста показываются быстрые переходы;
* при вводе показываются подсказки по разделам и маршрутам;
* текущий frontend-поиск использует данные из `frontend/lib/navigation.ts`;
* подключение backend-поиска выполняется только отдельной задачей;
* после подключения backend навигационные подсказки должны сохраниться как отдельная группа результатов;
* поисковый popup должен иметь ограниченную высоту и собственный вертикальный скролл;
* пустой результат должен отображаться явно и не подменяться фиктивными данными.

Границы ответственности:

### Platform Topbar

Может содержать только глобальные элементы:

* название активного раздела;
* навигацию раздела;
* глобальный поиск;
* глобальное создание сущности;
* адаптивное меню навигации раздела.

### Page Header и Page Toolbar

Должны оставаться внутри конкретной страницы и могут содержать:

* название страницы, документа или сущности;
* breadcrumbs;
* статус документа;
* локальные фильтры;
* режимы отображения;
* действия над текущей сущностью;
* сохранение, редактирование, удаление и проведение документа.

Локальные страницы не должны создавать собственную альтернативную глобальную верхнюю панель.

Без отдельного прямого задания пользователя запрещено изменять:

* высоту topbar;
* фон, нижнюю границу, радиусы и основную типографику;
* расположение названия раздела;
* desktop, tablet и mobile breakpoint-поведение;
* active, hover и focus-состояния;
* способ определения активного раздела и маршрута;
* общую структуру dropdown;
* полноширинный режим глобального поиска;
* расположение кнопок поиска и создания;
* компактное меню tablet/mobile;
* JSX-каркас `TopNavigation`;
* разделение между Platform Topbar и Page Toolbar;
* существующий визуальный язык SPORT-LEAD.

Разрешено без изменения дизайна:

* добавлять новые маршруты через `frontend/lib/navigation.ts`;
* добавлять новые группы и подпункты навигации;
* подключать backend к глобальному поиску отдельной задачей;
* добавлять новые типы результатов поиска;
* подключать реальную бизнес-логику к кнопке «Создать»;
* исправлять подтверждённые функциональные ошибки;
* улучшать доступность без изменения утверждённого внешнего вида;
* оптимизировать производительность без визуальных изменений.

Правила изменения навигации и поиска:

1. Источником разделов и маршрутов является `frontend/lib/navigation.ts`.
2. Нельзя вручную дублировать маршруты внутри `top-navigation.tsx`.
3. Новый пункт должен добавляться в существующую логическую группу.
4. Новый верхнеуровневый раздел создаётся только для отдельного крупного модуля.
5. Длинные названия должны сокращаться многоточием и не ломать layout.
6. Dropdown и popup не должны обрезаться родительским overflow.
7. Topbar должен проверяться совместно с expanded и compact режимами sidebar.
8. Глобальный поиск не должен подменять ошибки backend локальными demo-результатами.
9. Page Toolbar не должен содержать глобальную навигацию платформы.
10. Любое изменение внешнего вида `DS-SHELL-02` требует отдельной задачи и визуального подтверждения пользователя.

После изменения topbar обязательно проверить:
...
В итоговом отчёте задач, затрагивающих Platform Shell, указывать:

* изменённые компоненты;
* изменённые маршруты и группы;
* результат проверки expanded sidebar;
* результат проверки compact sidebar;
* результат проверки desktop, tablet и mobile;
* результат проверки dropdown;
* результат проверки глобального поиска;
* подтверждение: `DS-SHELL-01 visual contract preserved`;
* подтверждение: `DS-SHELL-02 visual contract preserved`.
