...
| `approved` | Финальный отчёт. Строки **иммутабельны**. Это кандидат в 1С (`16.2`). |
...
Выгрузка в 1С — Stage `16.2`, не этот срез. Контракт: в 1С уходит **последняя `approved`** версия.
