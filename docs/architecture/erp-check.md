...
- `[~]` `SalesOrderItem` CRUD, snapshot-наименование, размеры (`size_range`), персонализация (`personalization`) и пересчёт суммы заказа через `Decimal/Numeric`; focused tests, TypeScript, lint и Alembic проходят, production build требует сетевой доступ к Google Fonts;
