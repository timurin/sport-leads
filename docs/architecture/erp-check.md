...
- `[x]` `SalesOrderItem` CRUD, snapshot-поля и пересчёт суммы заказа через `Decimal/Numeric`;
...
**Evidence:** `backend/app/models/sales.py`, `backend/app/api/leads.py`, `backend/app/api/orders.py`, `backend/app/api/organizations.py`, `backend/app/services/lead_conversion.py`, `backend/app/services/sales_order_items.py`, `backend/tests/test_lead_conversion.py`, frontend sales routes/components и соответствующие Alembic migrations.
