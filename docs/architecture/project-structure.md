...
`SalesOrderItem` — коммерческая позиция: snapshot-наименование, размеры, персонализация, единица, количество, цена и сумма строки. `Material` не является номенклатурой заказа. В текущем контуре `size_range` и `personalization` сохраняются как nullable snapshot-поля.
