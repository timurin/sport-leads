...
`SalesOrderItem` — коммерческая позиция: snapshot-наименование, размеры, персонализация, цвет, единица, количество, цена и сумма строки. `Material` не является номенклатурой заказа. В текущем контуре `size_range`, `personalization` и `color` сохраняются как nullable snapshot-поля.
