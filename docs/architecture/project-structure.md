...
`frontend/components/settings/nomenclature-workspace.tsx` реализует рабочее место v0.8.5: дерево категорий, команды создания, поиск, фильтры, выбор вложенных категорий и ссылки на карточки. `frontend/components/settings/nomenclature-card.tsx` и маршрут `/settings/catalogs/nomenclature/[nomenclatureId]` реализуют view/edit карточки и переиспользуют typed custom fields без второго источника истины. Полный аудит, массовые действия, сохранение колонок, глобальные права, characteristics/variants и media остаются planned; последние перенесены в v0.8.6/v0.8.7.
