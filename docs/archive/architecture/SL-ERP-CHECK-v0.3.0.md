...
Архивная копия. Канонические документы: `docs/roadmap/roadmap.md`, `docs/architecture/project-structure.md` и `docs/architecture/erp-check.md`.

Связанный канонический документ: [roadmap](../../roadmap/roadmap.md). Этот файл сохранён только для истории.
...
## Целевая ERP-структура и производственная цепочка
...
Сквозная цепочка: заказ → позиции → номенклатура → модель/артикул → размеры и характеристики → партия → утверждённая спецификация → печать/раскрой/пошив → фактический расход и выпуск → 1С.
...
**Назначение:** хранить коммерческий документ с клиентом, организацией, коммерческими позициями, расчётами, оплатами и связью с производством.
**Текущий статус:** `[~]` список, карточка, организация, статусы/история и минимальные коммерческие позиции готовы; оплаты, документы и production workflow отсутствуют.
...
- `[~]` товарные позиции, цены и суммы строки постоянны; размеры, персонализация, скидки и НДС отсутствуют;
- `[ ]` план и факт оплат;
...
`SalesOrderItem` — коммерческая строка с snapshot-наименованием, единицей, количеством, ценой и суммой. `Material` не является номенклатурой заказа; связь с будущей номенклатурой добавляется отдельным архитектурным этапом.

## 4. Продукция

**Назначение:** вести номенклатуру готовых изделий, моделей, материалов, услуг, полуфабрикатов и комплектов.
**Текущий статус:** `[~]` материал реализован как отдельный каталог, общая номенклатура и модели отсутствуют.
...
Целевая последовательность модуля: номенклатура → модели/артикулы → размеры и характеристики → лекала → операционные узлы → нормативная технология.
...
Целевая последовательность: производственные партии → спецификации партий → утверждение версии → печать → раскрой → пошив → ОТК → фактический расход → выпуск. Спецификация — плановый состав до запуска партии, производственный факт хранится отдельно.
...
Будущий обмен с 1С должен включать номенклатуры, заказы, утверждённые спецификации, списание материалов, выполненные операции, выпуск продукции и связанные документы.
...
- `[x]` `SalesOrderItem` не связан с `Material`; номенклатура заказа не подменяется каталогом материалов;
- `[x]` контракт позиции ограничен `snapshot_name`, `unit`, `quantity`, `unit_price`, `line_amount`, при этом CRUD и пересчёт `SalesOrder.amount` сохранены;
- `[x]` миграция `d9e0f1a2b345` удаляет связь и переименовывает колонки, downgrade возвращает прежнюю структуру;
- `[~]` Product, Nomenclature, Specification, размеры, персонализация, скидки и НДС не входят в patch.
...
- `[x]` отдельный patch удалил ошибочную связь с `Material`; будущий Product/Nomenclature-каталог, production, склад и документы не затронуты.
