...
#### 13.1.2 — Purchase orders / Заказы поставщикам

- [x] 13.1.2.1 — Domain contract: `PurchaseOrder` + lines; draft = заявка; hosts `/purchases/orders` — `v1.00` `2026-08-29`; ADR-034; `SL-PURCHASE-ORDER-v1`; task `v1.00-stage-13.1.2-purchase-orders.md` / Контракт; без БД
- [x] 13.1.2.2 — SQLAlchemy + Alembic for `PurchaseOrder` + `PurchaseOrderLine` / SQLAlchemy + Alembic ЗП + строки — `v1.00` `2026-08-29`; Alembic `r7s8t9u0v123`; `test_purchase_orders_13_1_2_2.py` / модели + миграция
...
### 26.11 — Tech-cards list + document tabs / Список техкарт + табы документа

> **EN:** `/production/tech-cards`: list/kanban as `?view=` filters (not two routes); PageToolbar icons for create/generate; copy + delete draft-only; column «Ответственный»; centered create modal with text nomenclature + catalog picker; fan-out model/assembly/routing. Document tabs rename/reorder. Owner-pull. Task: `docs/tasks/v1.00-stage-26-owner-findings.md`. Contract: `SL-TECH-CARDS-LIST-26-11-v1`.
>
> **RU:** Список и канбан — виды одного маршрута. Иконки Создать / Сформировать. Копия и удаление черновика. Колонка Ответственный. Модалка создания по центру. Табы документа: Персонализация → Материалы → Операции → Пошив → Маршрут.

**Execute after / Исполнять после:** only when the owner names a code. Do not interrupt `12.5`. / только по явному коду владельца. Не прерывать `12.5`.

- [x] 26.11.1 — Contract (`SL-TECH-CARDS-LIST-26-11-v1`): view list/kanban; copy = new draft contour B; DELETE draft-only; create center modal; nomenclature text + picker; tab labels — `v1.00` `2026-08-29`; roadmap + HTML twin; task owner-findings / Контракт списка и табов
- [x] 26.11.2 — `?view=list|kanban`; redirect `/production/kanban`; nav hrefs — `v1.00` `2026-08-29`; `parseTechCardListView`; one top-nav «Техкарты» / Вид списка и канбана
- [x] 26.11.3 — PageToolbar: Техкарты/Канбан segment; Create + Generate as IconButton + tooltip — `v1.00` `2026-08-29`; owner visual OK список / Иконки тулбара
- [x] 26.11.4 — List column «Ответственный» (`responsible_name`) — `v1.00` `2026-08-29`; embed on list DTO / Колонка Ответственный
- [x] 26.11.5 — `DELETE /technical-cards/{id}` draft-only (409 иначе) + FE — `v1.00` `2026-08-29`; `test_tech_card_copy_delete_26_11.py` / Удаление черновика
- [x] 26.11.6 — `POST /technical-cards/{id}/copy` → new draft (XOR contour B) + FE — `v1.00` `2026-08-29`; `tech_card_copy_delete.py` / Копирование ТК
- [x] 26.11.7 — Create standalone: centered modal (not right drawer) — `v1.00` `2026-08-29`; `CreateDrawer` `variant="center"` / Модалка создания по центру
- [x] 26.11.8 — Create nomenclature: text + edit/cancel/save/catalog icons — `v1.00` `2026-08-29`; chrome as `26.3.5` / Поле номенклатуры
- [x] 26.11.9 — Catalog picker: reuse `NomenclaturePickModal` (one PRODUCT) — `v1.00` `2026-08-29` / Справочник номенклатур
...
| `2026-08-29` | New Stage **26.11** tech-cards list + document tabs (`26.11.1` closed contract; `26.11.2`–`26.11.12` open; owner-pull); `SL-TECH-CARDS-LIST-26-11-v1` | Новый **26.11**: список/канбан/копия/удаление/табы |
| `2026-08-29` | Closed **26.11.2**–`26.11.12`: list/kanban view, copy/delete draft, center create + picker fan-out, manager tabs; owner visual pending `26.11.3`/`26.11.10`/`26.11.11` | Закрыт **26.11**: код списка/табов; owner visual pending |
| `2026-08-29` | Owner visual OK `26.11.3` / `26.11.10` / `26.11.11` (list toolbar, create modal, document tabs); Stage **26.11** complete | Owner visual OK **26.11**; блок закрыт |
