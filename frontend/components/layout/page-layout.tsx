...
export function PageContent({ children, className = "", size = "default", width = "wide" }: {
  ...
  const spacing = { compact: "p-3 sm:p-4", default: "p-4 lg:p-6", spacious: "p-5 lg:p-8" }[size];
...
export function PageActions({ children, className = "", align = "end" }: {
  ...
  return <div className={`flex min-w-0 flex-wrap items-center gap-2 ${alignment} ${className}`}>{children}</div>;
}
...
export function ResponsiveGrid({ children, className = "", minItemWidth = "medium", gap = "default" }: {
  ...
  const gapClass = { compact: "gap-2", default: "gap-3", spacious: "gap-4 lg:gap-6" }[gap];
