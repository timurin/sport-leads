    ...
    <NomenclatureSectionCreateHost categories={categories}>
      <div className="space-y-portal-6 p-portal-6">
        <form
          ...
          className="grid gap-portal-2 rounded-portal-lg border border-portal-border bg-portal-surface p-portal-5 shadow-portal-card md:grid-cols-5"
        >
          <Input name="code" required pattern="[a-z0-9][a-z0-9_-]*" placeholder="Код реквизита" />
            ...
            <tbody>
              {fields.map((field) => (
                <tr key={field.id} className="border-b border-portal-border last:border-0">
                  ...
                  <td className="px-portal-4 py-portal-3">{field.is_active ? "Активен" : "Отключён"}</td>
